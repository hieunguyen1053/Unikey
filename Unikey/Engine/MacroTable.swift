...
public class MacroTable: ObservableObject {
  ...
  private var macroFilePath: URL {
    ...
    let unikeyDir = appSupport.appendingPathComponent("Unikey", isDirectory: true)

    // Create directory if needed
    if !fileManager.fileExists(atPath: unikeyDir.path) {
      try? fileManager.createDirectory(at: unikeyDir, withIntermediateDirectories: true)
    }
  ...
  public func lookup(key: String) -> String? {
    ...
    if let result = macros.first(where: { $0.key.lowercased() == keyLower })?.text {
  ...
  public func addItem(key: String, text: String) {
    ...
    if let existingIndex = macros.firstIndex(where: { $0.key.lowercased() == key.lowercased() }) {
      // Update existing
  ...
  public func loadFromFile(_ fname: String) -> Bool {
    ...
    for line in lines {
      ...
      if let colonIndex = trimmed.firstIndex(of: ":") {
        ...
        let text = String(trimmed[trimmed.index(after: colonIndex)...]).trimmingCharacters(
          in: .whitespaces)
        ...
        let text = String(trimmed[trimmed.index(after: equalIndex)...]).trimmingCharacters(
          in: .whitespaces)
        if !key.isEmpty && !text.isEmpty {
          macros.append(MacroItem(key: key, text: text))
        }
  ...
  public func importFromText(_ text: String) -> Int {
    ...
    for line in lines {
      ...
      if let colonIndex = trimmed.firstIndex(of: ":") {
        ...
        value = String(trimmed[trimmed.index(after: colonIndex)...]).trimmingCharacters(
          in: .whitespaces)
      } else if let tabIndex = trimmed.firstIndex(of: "\t") {
        key = String(trimmed[..<tabIndex]).trimmingCharacters(in: .whitespaces)
        value = String(trimmed[trimmed.index(after: tabIndex)...]).trimmingCharacters(
          in: .whitespaces)
      } else if let equalIndex = trimmed.firstIndex(of: "=") {
        key = String(trimmed[..<equalIndex]).trimmingCharacters(in: .whitespaces)
        value = String(trimmed[trimmed.index(after: equalIndex)...]).trimmingCharacters(
          in: .whitespaces)
      }
