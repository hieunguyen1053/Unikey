...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Auto restore keys for invalid words
    @AppStorage("UniKeyAutoRestore") var autoRestore: Bool = false

    /// Modern style tone placement (hoà, uý instead of hòa, úy)
    @AppStorage("UniKeyModernStyle") var modernStyle: Bool = false

    /// Free marking (allow tones at any position)
    @AppStorage("UniKeyFreeMarking") var freeMarking: Bool = true
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        // Observe settings changes to update engine
        NotificationCenter.default.addObserver(
            self,
    ...
    private func setupEventTap() {
        ...
        // Sync options from AppStorage to engine
        syncEngineOptions()
    ...
    /// Sync settings from AppStorage to engine
    private func syncEngineOptions() {
        eventTap?.updateOptions(
