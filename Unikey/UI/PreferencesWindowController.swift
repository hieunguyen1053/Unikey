...
class PreferencesWindowController: NSWindowController {
    ...
    private func loadPreferences() {
        ...
        // Checkboxes
        enableSpellCheckCheckbox?.state =
            defaults.bool(forKey: Keys.spellCheck) ? .on : .off
        freeMarkingCheckbox?.state =
            defaults.bool(forKey: Keys.freeMarking) ? .on : .off
        modernStyleCheckbox?.state =
            defaults.bool(forKey: Keys.modernStyle) ? .on : .off
    }

    func savePreferences() {
        defaults.set(
            inputMethodPopup?.indexOfSelectedItem ?? 0,
            forKey: Keys.inputMethod
        )
        defaults.set(
            enableSpellCheckCheckbox?.state == .on,
            forKey: Keys.spellCheck
        )
        defaults.set(
            freeMarkingCheckbox?.state == .on,
            forKey: Keys.freeMarking
        )
        defaults.set(
            modernStyleCheckbox?.state == .on,
            forKey: Keys.modernStyle
        )
...
extension Notification.Name {
    static let inputMethodChanged = Notification.Name(
        "UniKeyInputMethodChanged"
    )
}
