...
struct MacroEditorView: View {
  ...
  private var filteredMacros: [MacroItem] {
    ...
    return macroTable.macros.filter {
      $0.key.lowercased().contains(lowercased) || $0.text.lowercased().contains(lowercased)
    }
  }
  ...
  private var headerView: some View {
    HStack(spacing: 16) {
      ...
      VStack(alignment: .leading, spacing: 2) {
        Text(localization.currentLanguage == .vietnamese ? "Bảng gõ tắt" : "Macro Table")
          .font(.title2)
          .fontWeight(.semibold)
      ...
      HStack {
        Image(systemName: "magnifyingglass")
          .foregroundColor(.secondary)
        TextField(
          localization.currentLanguage == .vietnamese ? "Tìm kiếm..." : "Search...",
          text: $searchText
        )
  ...
  private var toolbarView: some View {
    HStack(spacing: 8) {
      ...
      Button {
        ...
        Label(localization.currentLanguage == .vietnamese ? "Thêm" : "Add", systemImage: "plus")
      }
      ...
      Button {
        if let selected = selectedMacros.first,
          let macro = macroTable.macros.first(where: { $0.id == selected })
        {
          editingMacro = macro
        }
      } label: {
        Label(localization.currentLanguage == .vietnamese ? "Sửa" : "Edit", systemImage: "pencil")
      }
      .buttonStyle(.bordered)
      ...
      Button {
        ...
        Label(localization.currentLanguage == .vietnamese ? "Xóa" : "Delete", systemImage: "trash")
      }
      .buttonStyle(.bordered)
      ...
      Button {
        ...
        Label(
          localization.currentLanguage == .vietnamese ? "Nhập" : "Import",
      ...
      Button {
        ...
        Label(
          localization.currentLanguage == .vietnamese ? "Xuất" : "Export",
      ...
      if !macroTable.macros.isEmpty {
        Button(role: .destructive) {
          ...
          Text(localization.currentLanguage == .vietnamese ? "Xóa tất cả" : "Clear All")
        }
        .buttonStyle(.bordered)
      }
    }
  ...
  private var tableView: some View {
    Table(filteredMacros, selection: $selectedMacros, sortOrder: $sortOrder) {
      TableColumn(
        localization.currentLanguage == .vietnamese ? "Từ viết tắt" : "Shortcut", value: \.key
      ) { macro in
        Text(macro.key)
      ...
      TableColumn(
        localization.currentLanguage == .vietnamese ? "Thay thế bằng" : "Replacement", value: \.text
      ) { macro in
        Text(macro.text)
          .lineLimit(2)
      }
    ...
    .contextMenu(forSelectionType: UUID.self) { selection in
        ...
        let macro = macroTable.macros.first(where: { $0.id == id })
      {
        Button {
          ...
          Label(
            localization.currentLanguage == .vietnamese ? "Sửa..." : "Edit...",
            systemImage: "pencil")
        }
      }
      ...
      if !selection.isEmpty {
        Button(role: .destructive) {
          ...
          Label(
            localization.currentLanguage == .vietnamese ? "Xóa" : "Delete", systemImage: "trash")
        }
      }
    ...
    VStack(spacing: 16) {
      ...
      if searchText.isEmpty {
        ...
        Button {
          ...
          Label(
            localization.currentLanguage == .vietnamese ? "Thêm từ viết tắt" : "Add Shortcut",
            systemImage: "plus")
        }
  ...
  private var footerView: some View {
    HStack {
      ...
      Button(localization.currentLanguage == .vietnamese ? "Đóng" : "Close") {
        dismiss()
      }
  ...
  private func importMacros() {
    ...
    panel.title =
      localization.currentLanguage == .vietnamese ? "Chọn file macro" : "Select Macro File"
  ...
  private func exportMacros() {
    ...
    panel.title = localization.currentLanguage == .vietnamese ? "Lưu file macro" : "Save Macro File"
...
struct MacroItemEditorSheet: View {
  ...
  var body: some View {
    VStack(spacing: 20) {
      ...
      Text(
        isEditing
          ? (localization.currentLanguage == .vietnamese ? "Sửa từ viết tắt" : "Edit Shortcut")
          : (localization.currentLanguage == .vietnamese ? "Thêm từ viết tắt" : "Add Shortcut")
      )
      .font(.headline)
      ...
      Form {
        TextField(
          localization.currentLanguage == .vietnamese ? "Từ viết tắt:" : "Shortcut:", text: $key
        )
        .textFieldStyle(.roundedBorder)
        .frame(width: 300)

        TextField(
          localization.currentLanguage == .vietnamese ? "Thay thế bằng:" : "Replacement:",
      ...
      // Buttons
      HStack {
        Button(localization.currentLanguage == .vietnamese ? "Hủy" : "Cancel") {
          dismiss()
        }
        ...
        Button(localization.currentLanguage == .vietnamese ? "Lưu" : "Save") {
          save()
        }
        .keyboardShortcut(.defaultAction)
        .disabled(
