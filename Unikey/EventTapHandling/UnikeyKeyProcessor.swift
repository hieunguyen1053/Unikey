...
public class UnikeyKeyProcessor {
    ...
    public func processKey(
        ...
        if let cv = charValue {
            ...
            if state.postponeKeyEv && state.postponeCount < 50 {
                ...
                debugCallback?(
                    "Postponing key event, count=\(state.postponeCount)"
                )
    ...
    private func processRegularKey(charValue: UInt32, flags: CGEventFlags)
        -> ProcessKeyResult
    {
        ...
        debugCallback?(
            "Engine result: backspaces=\(backspaces), bufChars=\(bufChars)"
        )
        ...
        if backspaces > 0 || bufChars > 0 {
            let outputText =
                bufChars > 0
                ? String(utf16CodeUnits: buffer, count: bufChars) : ""
    ...
    private func processBackspace() -> ProcessKeyResult {
        ...
        debugCallback?(
            "Backspace result: backspaces=\(backspaces), bufChars=\(bufChars)"
        )
    ...
    public func handleShortcut(keyCode: UInt16, flags: CGEventFlags) {
        ...
        case 0x65:  // F9 - Toggle
            ...
            unikey.resetBuf()
            debugCallback?(
                "Vietnamese mode: \(state.vietnameseEnabled ? "ON" : "OFF")"
            )
        default:
            break
        }
