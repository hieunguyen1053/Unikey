...
public class UnikeyTextCommitter {
  ...
  public func inject(backspaceCount: Int, text: String, proxy: CGEventTapProxy) {
  ...
  private func sendBackspaceEvent(proxy: CGEventTapProxy, keyUpToo: Bool = true) {
    ...
    guard
      let keyDown = CGEvent(
        keyboardEventSource: source, virtualKey: backspaceKeyCode, keyDown: true)
    else {
      return
    }

    // Mark as Unikey event
    keyDown.setIntegerValueField(.eventSourceUserData, value: UnikeyTextCommitter.eventMarker)
    keyDown.tapPostEvent(proxy)

    if keyUpToo {
      guard
        let keyUp = CGEvent(
          keyboardEventSource: source, virtualKey: backspaceKeyCode, keyDown: false)
      else {
        return
      }
      keyUp.setIntegerValueField(.eventSourceUserData, value: UnikeyTextCommitter.eventMarker)
